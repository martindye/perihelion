const qid = 'M+8';
const r = await fetch('https://simbad.u-strasbg.fr/simbad/sim-id?Ident=' + qid);
console.log('status:', r.status, 'url:', r.url);
const t = await r.text();
console.log('len:', t.length);
const i = t.indexOf('(ep=J2000)');
console.log('ep=J2000 at:', i);
if (i >= 0) {
  const seg = t.slice(i, i + 1600);
  const m = seg.match(/(\d{1,2})\s+(\d{2})\s+([\d.]+)\s+([+-])\s*(\d{2})\s+(\d{2})\s+([\d.]+)/);
  console.log('regex match:', m ? m.slice(1) : 'NO MATCH');
  console.log('SEG HEAD:', JSON.stringify(seg.slice(0, 400)));
} else {
  console.log(t.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').slice(0, 600));
}

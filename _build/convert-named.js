...
/* mission-card data: distance (pc) + V magnitude per HIP (HYG) */
const starDist = new Map();
...
for (let li = 1; li < hygLines.length; li++) {
  ...
  const distPc = parseFloat(f[6]);
  if (isFinite(distPc) && distPc > 0) starDist.set(hip, [distPc, parseFloat(f[5]) || null]);
 ...
 *   P.starDist        - [[hip, distancePc, Vmag]] per named star (HYG) — journey
 */
'use strict';
window.P = window.P || {};
...
P.starDist = { count: ${starDist.size}, data: ${JSON.stringify([...starDist].sort((a, b) => a[0] - b[0]))} };
`;

...
| Bayer + Flamsteed designations for every classical star (≈2,900 stars, e.g. `tau Ceti`, `61 Cygni`) | **HYG 4.2** (Hybrid Star Catalog — Hipparcos + Yale Bright Star + Gliese; <https://astronexus.com/hyg>), **CC BY-SA 4.0**. Raw download `_build/hygdata_v42.csv` is not committed; the distilled `_build/hyg-bayer-flam.csv` is. Generated into `js/stars-named.js` by `_build/convert-named.js`. |
...
| Pluto (New Horizons global map) | **done**: [Wikimedia — "Pluto-map-sept-16-2015.jpg"](https://commons.wikimedia.org/wiki/File:Pluto-map-sept-16-2015.jpg) (8192×4096 equirect, New Horizons LORRI/Ralph composite, US Gov PD), downloaded at 2048px | `_qa/_minor_pluto_full.jpg` (scratch) — embedded as `pluto` (2048×1024) in `js/planets-textures.js` |
| Ceres, Haumea, Makemake, Eris (2K, "fictional" art from real data) | **done**: SST fictional set — `2k_ceres_fictional.jpg`, `2k_haumea_fictional.jpg`, `2k_makemake_fictional.jpg`, `2k_eris_fictional.jpg` ([Solar System Scope](https://www.solarsystemscope.com/textures/), CC-BY-4.0) | `_qa/_minor_{ceres,haumea,makemake,eris}.jpg` (scratch, 1024×512) — embedded in `js/planets-textures.js` |
| Vesta, Pallas, Hygiea, Ixion + 11 major moons (Io…Charon) | **no public 2K equirect maps exist at this scale** — deterministic procedural maps (seeded mottle + body-specific features: Io's sulfur volcanoes, Europa's lineae, Iapetus' two-tone face, Phobos' cratering…), generated by `_qa/gen-minor-textures.mjs` | `_qa/_minor_*.jpg` (scratch) — embedded in `js/planets-textures.js` |
